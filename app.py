...
# Inicialização dos Bancos de Dados na memória para reaproveitamento
...
# Abas do Menu Principal Completas e Corrigidas
tabs = st.tabs([
...
with tabs[1]:
    ...
    with col_or2:
        ...
        # Lógica matemática de frete técnico avançado K&G
        ...
        st.metric("Custo Final do Frete", f"R$ {valor_frete_final:.2f}", f"Estrada de terra: +R$ {taxa_rural:.2f}")
    ...
    if st.button("🖨️ Gerar Espelho da Proposta Comercial"):
        st.markdown(f"""
            <div class="print-box">
                ...
                <b>Data de Emissão:</b> {datetime.now().strftime('%d/%m/%Y')} | <b>Data de Entrega:</b> {c_data_festa.strftime('%d/%m/%Y')} às {c_horario.strftime('%H:%M')}<br>
                -------------------------------------------------------------------------<br>
                <b>DADOS DO CLIENTE:</b><br>
                - Nome: {c_nome} | WhatsApp: {c_whats}<br>
                - E-mail: {c_email}<br>
                -------------------------------------------------------------------------<br>
                ...
                -------------------------------------------------------------------------<br>
                *Garantia de pontualidade, estrutura e sabor inigualáveis K&G.*
            </div>
        """, unsafe_allow_html=True)
...
with tabs[2]:
    ...
    st.markdown(f"<div class='alerta-aniv'>🎉 <b>CENTRAL DE RELACIONAMENTO K&G:</b> Aniversariante do Mês detectado: <b>Juliana Mendes Rossi (12/{mes_atual})</b>. Mande um abraço de luxo!</div>", unsafe_allow_html=True)
    st.data_editor(pd.DataFrame([
        {"Cliente VIP": "Juliana Mendes Rossi", "WhatsApp": "(41) 99123-4567", "Aniv. Cliente": f"12/{mes_atual}", "Aniv. Marido": "18/10", "Aniv. Filhos": "Gabriel (04/02)", "Pedidos Anteriores": "KG-2026-1042", "🚨 RESTRIÇÕES": "NÃO PODE CONTER AMENDOIM!"}
...
# --- ABA 5: PRODUTOS COMPLETOS & CALCULADORA IFOOD MANTIDA ---
with tabs[5]:
    ...
    with c_v1: st.markdown(f"<div class='preco-box'><b>🛍️ PREÇO BALCÃO</b><br><span style='font-size:24px; font-weight:bold;'>R$ {preco_balcao:.2f}</span><br>Lucro Protegido de {margem_desejada}%</div>", unsafe_allow_html=True)
    with c_v2: st.markdown(f"<div class='preco-box' style='background:#901414;'><b>🛵 PREÇO IFOOD</b><br><span style='font-size:24px; font-weight:bold;'>R$ {preco_ifood:.2f}</span><br>Taxas do App Cobertas</div>", unsafe_allow_html=True)

# --- TRATAMENTO ACESSÓRIO DAS DEMAIS ABAS PRESERVADAS ---
...
with tabs[9]: st.write("Fornecedores Principais de Curitiba (BWB, Central do Chocolate CWB, Nova Íris, Plassete).")
